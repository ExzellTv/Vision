...
- **Feasibility score source of truth.** Dashboard land metrics come from the selected project `plot`, not the floor-plan quality score. If `plot` is missing, Dashboard shows `N/A` for feasibility score, acres, and acquisition cost. If a selected plot has a stale or missing `plot.feasibility_score`, Dashboard and Feasibility recompute with `runValuation()` and persist the refreshed score back to that same project's `plot.feasibility_score`.
- **Plot ownership isolation.** `/feasibility` is context-driven rather than route-scoped, so project activation must call `resetProject()` before loading another project's state. Feasibility also guards async fetch/update responses against the current `project.projectId`, resets local map/plot state when the active project changes, restores a saved plot only once per project, and treats city search/map clicks as local analysis until the user explicitly clicks **Select Plot**. This prevents same-city projects from overwriting each other's saved land parcel.
- **Feasibility CTA visibility.** In `FeasibilityDashboard.jsx`, the analysis side uses a split layout: the upper analysis content scrolls, but the CTA stack (`Choose Builders`, `Select Plot`, `Generate PDF Report`) lives in a persistent footer inside the same panel. Keep that footer always visible on desktop and on the mobile `Analysis` tab; do not put those actions back inside the scrolling content area.
- **Dashboard recent project activation.** `Dashboard.jsx` Recent Projects rows carry the original Mongo project as `sourceProject`. Clicking a row resets the shared project store, hydrates id/name/location/generate params/materials/building context/schedule/floor plans from that exact project, then navigates to `/feasibility`. This fixes the bug where recent-project clicks reused stale persisted location context (for example Austin, Texas) instead of opening feasibility for the clicked project. Verified with `git diff --check` and `npm run build` in `frontend/` (build passes; Vite still warns about large chunks).
- **Regenerate Floor Plan fixes.** Three bugs were patched in `FloorPlanEditor.jsx`: (1) The regenerate button now calls `setZoom(1)` and `setPanOffset({ x: 0, y: 0 })` so any prior pan/zoom doesn't carry over to the new plan. (2) `generateLocalFloorPlan` now runs a post-layout bounds check — after all rooms (including garage, stairs) are placed it computes the actual `maxRoomX`/`maxRoomY` extents and grows `plan.width`/`plan.depth` to contain them; this prevents rooms from drawing outside the bounding box when floating-point strip division or the min-6 ft guard pushed room edges beyond the declared dimensions. (3) The canvas padding in `renderFloorPlan` changed from a fixed `60` to `Math.min(60, Math.floor(Math.min(cw, ch) * 0.08))` — on mobile the padding now scales down to ~8% of the shorter canvas dimension so the auto-fit scale has enough room to render the plan visibly at all viewport sizes.
