...
function WidgetCard({ title, help, children, style = {} }) {
  return (
    ...
    >
      <h3 style={{ margin: "0 0 16px 0", fontSize: "1rem", color: colors.textBright, fontWeight: "bold", letterSpacing: "-0.01em", display: "inline-flex", alignItems: "center", gap: 6, whiteSpace: "normal", wordWrap: "break-word" }}>
        {title}
...
// ─── RequestExpansion ─────────────────────────────────────────────────────────
function RequestExpansion({ project, tourFirst, isMobile }) {
...
// ─── DesktopRequestCard ───────────────────────────────────────────────────────
function DesktopRequestCard({ req, expandedId, handleExpand, approveRequest, denyRequest, tourFirst }) {
  return (
    <div
      data-tour={tourFirst ? "request-row" : undefined}
      style={{
        ...card,
        borderRadius: radii.md,
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <div style={{ display: "flex", gap: "8px" }} onClick={(e) => e.stopPropagation()}>
            <button
              ...
              data-tour={tourFirst ? "approve-btn" : undefined}
      ...
      {expandedId === req.id && (
        <RequestExpansion project={req} isMobile={false} tourFirst={tourFirst} />
      )}
    </div>
  );
}
